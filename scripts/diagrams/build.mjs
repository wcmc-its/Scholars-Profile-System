...
function buildHtml(items) {
  ...
  const PROJECT = "Scholars Profile System";      // → hero <h1>, <title>, og:title
  const ORG = "Weill Cornell Medicine · ITS";     // → hero kicker (small line above the title)
  const HERO =                                    // → hero paragraph (one for a newcomer)
    "The architecture of the Scholars Profile System — its <b>system context</b>, the " +
    "<b>application &amp; AWS topology</b>, the app's <b>internal components</b>, the " +
    "<b>network topology</b>, the open <b>edge-topology decision</b>, and the " +
    "<b>grant-matching subsystem</b> (opportunity corpus supply chain, ingest cadences and " +
    "freshness guardrails, and the Matcha matching surfaces) — for ITS and cloud-team review.";
  const META_CHIPS = [                            // → hero pills (key facts at a glance)
    "AWS CDK · nine stacks (ADR-008)", "Staging &amp; Production · one shared account/VPC, per-env SGs",
    "us-east-1 (DR us-west-2)", "Grant matching · ReciterAI corpus + Matcha engines",
  ];
  const docDesc =                                 // → <meta description> + OG (one sentence)
    `Architecture of ${PROJECT} — what it is, what it does, and how it's deployed.`;
  // ─────────────────────────────────────────────────────────────────────────────
