...
console.log(`    verdict mix: win=${pct(vc.win)}  neutral=${pct(vc.neutral)}  noise=${pct(vc.noise)}`);
...
for (const tier of ["S", "A", "B"]) {
  ...
  const w = g.filter(isWin).length, bn = g.filter((r) => r.consensusBeatsPlainName).length;
  ...
  console.log(`     ${tier}   | ${String(g.length).padStart(3)} | ${String(Math.round(100*w/g.length)).padStart(3)}% |    ${String(Math.round(100*bn/g.length)).padStart(3)}%    |        ${String(Math.round(100*se/g.length)).padStart(3)}%         |    ${med}`);
}
// pub_count cut as an alternative opacity signal
for (const [lab, lo, hi] of [["pubN=1", 1, 1], ["pubN 2-4", 2, 4], ["pubN>=5", 5, 1e9]]) {
  const g = J.filter((r) => r.toolPubCount >= lo && r.toolPubCount <= hi);
  if (!g.length) continue;
  const w = g.filter(isWin).length, bn = g.filter((r) => r.consensusBeatsPlainName).length;
  console.log(`    ${lab.padEnd(9)}| ${String(g.length).padStart(3)} | ${String(Math.round(100*w/g.length)).padStart(3)}% |    ${String(Math.round(100*bn/g.length)).padStart(3)}%    |`);
}
...
console.log(`    mechanical fragmentStart flagged: ${pct(fs1.length)};  judge-broken: ${pct(jbroken.length)}`);
console.log(`    precision P(judge-broken | flagged) = ${fs1.length ? Math.round(100*tp1/fs1.length) : 0}%`);
console.log(`    recall    P(flagged | judge-broken) = ${jbroken.length ? Math.round(100*tp1/jbroken.length) : 0}%`);
console.log(`    COLLATERAL — among fragmentStart-flagged rows: win=${fs1.filter(isWin).length} neutral=${fs1.filter(r=>r.consensusVerdict==="neutral").length} noise=${fs1.filter(isNoise).length}`);
...
console.log(`    mechanical foilCueNearby: ${pct(foilFlag.length)};  judge tool_is_foil: ${pct(jfoil.length)}`);
console.log(`    precision P(judge-foil | flagged) = ${foilFlag.length ? Math.round(100*tp4/foilFlag.length) : 0}%   recall = ${jfoil.length ? Math.round(100*tp4/jfoil.length) : 0}%`);
// name-position signal: among judged foils, where does the tool name sit?
const npFoil = jfoil.filter((r)=>r.namePosition!=null).map((r)=>r.namePosition).sort((a,b)=>a-b);
const npWin  = J.filter(isWin).filter((r)=>r.namePosition!=null).map((r)=>r.namePosition).sort((a,b)=>a-b);
const median = (a)=>a.length?a[Math.floor(a.length/2)]:null;
console.log(`    median namePosition: judged-foils=${median(npFoil)}  wins=${median(npWin)}  (higher=name later in sentence)`);
...
console.log(`      of those: win=${reused.filter(isWin).length} neutral=${reused.filter(r=>r.consensusVerdict==="neutral").length} noise=${reused.filter(isNoise).length}`);
...
for (const k of Object.keys(fm).sort((a,b)=>fm[b]-fm[a])) console.log(`    ${k.padEnd(24)} ${pct(fm[k])}`);
...
console.log(`    fragmentStart prevalence: ${pct(popFrag, full.length)}  | foil: ${pct(popFoil, full.length)} | reused: ${pct(popReuse, full.length)}`);
...
const show = (label, rows, k = 4) => {
  ...
  for (const r of rows.slice(0, k)) console.log(`      [${r.salienceTier}|pubN${r.toolPubCount}] ${r.tool} — ${r.consensusVerdict}/${r.consensusFailureMode}\n         "${clip(r.snippet)}"`);
};
console.log(`\n[7] worked examples:`);
show("WINS (opaque tool, snippet beats name)", J.filter((r)=>isWin(r) && !r.nameIsSelfExplanatory));
show("FAILURE #1 well-known name, snippet=one-paper result", J.filter((r)=>r.consensusFailureMode==="well_known_name_clear"));
show("FAILURE #2 broken fragment", J.filter((r)=>r.consensusFailureMode==="broken_fragment"));
show("FAILURE #4 tool is a foil", J.filter((r)=>r.consensusFailureMode==="tool_is_foil"));
