 ...
 * (People — withheld above SCHOLAR_EXPORT_CAP — Publications, Criteria). Same query string as the page
...
export async function GET(request: Request) {
  ...
  const filename = `Top clinical journal publications ${params.from}-${params.to} ${generatedAt.toISOString().slice(0, 10)}.xlsx`;
