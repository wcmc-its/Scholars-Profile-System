/**
 * `/edit/profiles` — the Profiles roster (#160 UI follow-up,
...
export const metadata = {
  title: "Profiles — Scholars Console",
...
export default async function EditScholarsPage({
  ...
  if (!session) {
    redirect("/api/auth/saml/login?return=/edit/profiles");
  }
  ...
  if (isEmptyScope(scope)) {
    const denial = requireSuperuserGet({ session, path: "/edit/profiles", targetId: "roster" });
    if (denial !== null) {
      return (
    ...
    loadDataQualityRoster(
      {
        ...
        includeHidden: params.includeStudents,
