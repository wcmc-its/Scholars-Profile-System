...
/**
 * Unit collaboration network — the interactive body of the center (#1137) and
 * department Collaboration tabs. The unit-specific wrappers
 * (`components/center/center-collaboration-tab.tsx`,
 * `components/department/department-collaboration-tab.tsx`) pass the data URL,
 * the export file slug and the group wording (`CollabVocab`: programs for a
 * center, divisions for a department); nothing here is unit-specific.
 *
 * Fetches the whole graph payload once from the unit's route, lazily loads
 * `vis-network` (kept off the unit page's initial bundle), and rebuilds the
 * DataSets in the browser on every control change via the pure helpers in
 * `lib/center-collaboration/graph.ts`. Controls: people↔group view, an axis
 * toggle (Publications / Grants / Both — #1137 Phase 2), group picker (one
 * group at a time), min co-pubs, year range, person search, hide-unconnected,
...
export function UnitCollaborationNetwork({
  dataUrl,
  exportSlug,
  unitName,
  vocab,
}: {
  /** The unit's collaboration route, e.g. `/api/departments/<slug>/collaboration`. */
  dataUrl: string;
  /** File-name stem for the PNG / HTML downloads (`<exportSlug>-collaboration.*`). */
  exportSlug: string;
  /** Unit display name — the standalone-HTML export title. */
  unitName: string;
  /** Group / unit wording (programs vs divisions). */
  vocab: CollabVocab;
}) {
  const [payload, setPayload] = useState<UnitCollaborationPayload | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(dataUrl)
      .then((r) => {
        ...
        return r.json() as Promise<UnitCollaborationPayload>;
      })
      .then((data) => {
        if (!alive) return;
        setPayload(data);
  ...
  const computed = useMemo(() => {
    ...
    if (view === "program") {
      ...
      for (const p of payload.programs) {
        ...
        visNodes.push({
          ...
          title: programTitle(p.label, pubInternal, grantInternal, axis, vocab),
        });
      }
        ...
        axis === "both"
          ? mergeAxisEdgesThresholded(pub.edges, grant.edges, minCoPubs)
              ...
              .map((e) => ({
                ...
                title: crossProgramTitle(e.pubWeight, e.grantWeight, vocab),
              }))
          : (axis === "grants" ? grant.edges : pub.edges)
              ...
              .map((e) => ({
                ...
                title:
                  axis === "grants"
                    ? `${e.weight} cross-${vocab.group} shared grant${e.weight === 1 ? "" : "s"}`
                    : `${e.weight} cross-${vocab.group} co-authored papers`,
              }));
      return {
        nodes: visNodes,
        edges: visEdges,
    ...
    for (const n of payload.nodes) {
      ...
      visNodes.push({
        ...
        title: nodeTitle(
          n.name,
          programMeta.label.get(key) ?? vocab.unclassifiedLabel ?? "Unclassified",
          n.pubCount,
          degree[n.i],
          axis,
          vocab,
  ...
  useEffect(() => {
    ...
    // propagates to the error boundary and blanks the whole unit page. A
  ...
  const exportPng = useCallback(() => {
    ...
    a.download = `${exportSlug}-collaboration.png`;
    a.click();
  }, [exportSlug]);
  ...
  const exportHtml = useCallback(() => {
    ...
    a.download = `${exportSlug}-collaboration.html`;
    a.click();
    URL.revokeObjectURL(a.href);
  }, [payload, computed, exportSlug, unitName]);
  ...
  if (
    ...
    return (
      <p className="py-10 text-sm text-[var(--color-text-tertiary)]">
        Not enough co-authorship data yet to draw a collaboration network for this {vocab.unitNoun}.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-md border border-[var(--color-border)] bg-[var(--color-surface,#fafafa)] p-3">
        <div className="inline-flex overflow-hidden rounded border border-[var(--color-border)]">
          {(["people", "program"] as View[]).map((v) => (
            ...
            >
              {v === "people" ? "People" : cap(vocab.groups)}
            </button>
          ))}
        </div>
      ...
      {/* Group picker — one program / division at a time (people view). Doubles
          as the color key. In the group rollup view it is just a static legend. */}
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs">
        <span className="text-[var(--color-text-tertiary)]">
          {view === "people" ? `Show ${vocab.group}:` : `${cap(vocab.groups)}:`}
        </span>
        {view === "people" && (
          <button
            type="button"
            onClick={() => setSelectedProgram("all")}
            aria-pressed={selectedProgram === "all"}
      ...
      <p className="text-[11px] text-[var(--color-text-tertiary)]">
        {computed.shown.toLocaleString()} {view === "people" ? vocab.memberNoun : vocab.groups} shown · node size ={" "}
        {axis === "grants"
          ? `within-${vocab.group} co-investigators`
          : axis === "both"
            ? `within-${vocab.group} collaborators`
            : `within-${vocab.group} co-authors`}{" "}
        · edge thickness ={" "}
        {axis === "grants" ? "shared grants" : axis === "both" ? "tie strength" : "shared publications"}
        {view === "people" && (
          <> · links are within-{vocab.group} (cross-{vocab.group} collaboration is in the {cap(vocab.groups)} view)</>
        )}
        {computed.omitted > 0 && (
          <> · {computed.omitted.toLocaleString()} paper{computed.omitted === 1 ? "" : "s"} with &gt;{MEMBER_CAP} {vocab.unitNoun} authors omitted from links</>
        )}
...
function nodeTitle(
  ...
  if (axis === "grants") {
    return `${name}\n${program} · ${degree} ${vocab.group} co-investigator${degree === 1 ? "" : "s"} shown`;
  }
  if (axis === "both") {
    return `${name}\n${program} · ${pubCount} publications · ${degree} collaborator${degree === 1 ? "" : "s"} shown`;
  }
  return `${name}\n${program} · ${pubCount} publications · ${degree} ${vocab.group} co-author${degree === 1 ? "" : "s"} shown`;
}
...
/** Group-rollup node tooltip — axis-aware within-group counts. */
function programTitle(
  ...
  const within = `within-${vocab.group}`;
  if (axis === "grants") {
    return `${label}\n${grantInternal} ${within} shared grant${grantInternal === 1 ? "" : "s"}`;
  }
  if (axis === "both") {
    return `${label}\n${pubInternal} ${within} papers · ${grantInternal} ${within} grants`;
  }
  return `${label}\n${pubInternal} ${within} co-authored papers`;
}

/** Group-rollup edge tooltip for the "Both" overlay. */
function crossProgramTitle(
  pubWeight: number,
  grantWeight: number,
  vocab: CollabVocab,
): string {
  const cross = `cross-${vocab.group}`;
  const parts: string[] = [];
  if (pubWeight > 0)
    parts.push(`${pubWeight} ${cross} paper${pubWeight === 1 ? "" : "s"}`);
  if (grantWeight > 0)
    parts.push(`${grantWeight} ${cross} grant${grantWeight === 1 ? "" : "s"}`);
  return parts.join(" · ");
}
