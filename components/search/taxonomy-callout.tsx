 ...
 * to 4 secondary rows + an optional "N more in Browse →" overflow row.
 *
    ...
    <div className="my-3">
      ...
      {hasDisclosure ? (
        <ul
          ...
          className={`${expanded ? "" : "hidden"} mt-1 flex flex-col gap-1 rounded-lg border border-[#d6e2ec] bg-[#eef4f9] px-3.5 py-2`}
        >
          {secondary.map((m) => (
          ...
          {overflowCount > 0 ? (
            <li>
              <Link
                ...
                className="inline-flex items-center gap-1 text-[12.5px] italic text-zinc-500 hover:text-[var(--color-primary-cornell-red)] hover:no-underline"
              >
...
function PrimaryContent({ match }: { match: TaxonomyMatch }) {
  return (
    <Link
      href={match.href}
      className="min-w-0 flex-1 text-zinc-600 no-underline hover:no-underline"
...
function PrimaryCta({ match }: { match: TaxonomyMatch }) {
  return (
    <Link
      ...
      tabIndex={-1}
      aria-hidden="true"
      className="shrink-0 whitespace-nowrap text-[13px] font-medium text-[var(--color-accent-slate)] no-underline transition-colors group-hover:text-[var(--color-primary-cornell-red)] hover:no-underline"
    >
...
function SecondaryRow({ match }: { match: TaxonomyMatch }) {
  return (
    <Link
      href={match.href}
      aria-label={ariaLabelFor(match)}
      className="group/row flex flex-wrap items-center gap-2 rounded px-1 py-1 text-[13px] no-underline hover:no-underline"
    >
