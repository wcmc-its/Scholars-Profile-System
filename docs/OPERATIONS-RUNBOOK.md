...
| 5 | Retracted paper showing (or a paper vanished) | The retraction **notice** is hidden; the **original article** keeps `Journal Article` type until restamped | Nightly `PubMedRetractions` step stamps the retracted PMID set → `publicationType='Retraction'` (then #63 hides), then reindexes. **Auto-runs in prod** (rules ENABLED since 2026-07-07) — manual `npm run etl:pubmed-retractions` **+ reindex** is normally a no-op. ⚠ If it stopped stamping, check the rule state first: prod is drifted (`etlSchedulesEnabled:false` in config, ENABLED live), so a `cdk deploy Sps-Etl-prod` can silently disable the nightly (#1512). → [`./retracted-publications.md`](./retracted-publications.md) |
