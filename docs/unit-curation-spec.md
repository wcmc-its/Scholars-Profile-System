...
- **`leaderInterim` on `Center`, historical** — Phase 1 added an in-row `Center.leaderInterim` column as the interim/acting qualifier for centers; #2542 contract A retired it in favor of `OrgUnitRoleAssignment.interim`, the same column that already carried the qualifier for `center_program` assignments. For departments/divisions `leaderInterim` remains a columnless `field_override` field (a column exists only where something writes one; nothing ETL-writes interim).
...
`CenterMembership` and `DivisionMembership` share a shape — `(unitCode, cwid, source, lastRefreshedAt)`, no FK to `Scholar` (ADR-003 — an added scholar may not have a row yet); UI-added rows carry `source='manual-ui'`. One center is the exception: the CTSC roster (center slug `ctsc`) is mirrored nightly by `etl/ctsc-roster` (#2780) with `source='ctsc-feed'` (a profiled scholar, keyed by CWID) and `source='ctsc-feed-external'` (a person with no profile; `cwid` holds an `ExternalMember.cuid` of the form `ctsc:<feed PrimaryKey>`). Those rows are feed-owned: /edit Remove returns `409 feed_owned_membership`, and the sync never touches rows with any other source. They are the manual-roster tables for the two manually-owned unit families. A roster operation targeting a department or an LDAP-sourced division is rejected `400` — there is no manual roster to write.
...
A `source='manual'` unit is untouched by the ETL by construction: the dept/div `upsert` loop iterates only LDAP-emitted codes; the orphan sweep is `source='ED'`-scoped; the only nightly step that touches a center is `CtscRoster`, which writes only the CTSC center's feed-sourced membership rows, never a `Center` row.
...
- **`etl/ed`** — gains the slug + leadership `field_override` precedence consult for departments/divisions. ADR-002 Path A/B detection is unchanged; **Path C (`data/division-chiefs.txt`) is retired** — its ETL read/write branch was deleted in #2560. The bullet below's same-PR backfill-then-delete ordering doesn't constrain this case: `data/division-chiefs.txt` was never tracked in the repo, so the Phase 9 backfill (`scripts/backfills/2026-06-10-import-unit-curation.ts`) found no file content to migrate, and there was no live data a later deletion could orphan.
...
- **Co-leadership.** One leader per unit (a single `OrgUnitRoleAssignment` leadership row, #2542 contract A) plus interim/vacancy; `OrgUnitRole.singleHolder` isn't enforced on the write path yet, so this is a convention, not a constraint. Co-Directors as a distinct, enforced concept remains deferred.
