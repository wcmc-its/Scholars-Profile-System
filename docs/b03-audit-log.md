...
- **Separate database** — the application role is granted `INSERT` and *nothing else* on `scholars_audit`, while retaining full DML on the application database. That asymmetric grant is what makes the log append-only: an `UPDATE` or `DELETE` against an audit row fails at the database. The `etl` role holds the identical `INSERT`-only grant (#2556), for the nightly ETL's auto-confirmed K=2 match audit write.
