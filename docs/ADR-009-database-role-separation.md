...
> **Implementation status (2026-05-31):** Phases 0–2 **landed** and are **deploy-confirmed on staging + prod** (the grant-equality verify passes for all four roles in both envs). **Phase 3 landed as code** (the seeder `app_rw` REVOKE + the DML-only golden); its live effect lands on the next `Sps-Data-<env>` deploy, now unblocked by Phase 2's confirmation per the sequencing rule below.
...
- **Phase 3 — tighten. ✅ LANDED (code; live on the next `Sps-Data-<env>` deploy).** The seeder now revokes `app_rw`'s `scholars`.* DDL ([`statements.ts` `appRwTightenStatements`](../cdk/lambda/db-bootstrap-seed/statements.ts) via [`seed.ts` `runAppRwTighten`](../cdk/lambda/db-bootstrap-seed/seed.ts), custom-resource `Revision` `2→3`): `REVOKE IF EXISTS CREATE, DROP, ALTER, INDEX, REFERENCES, EXECUTE, TRIGGER ON \`scholars\`.* FROM 'app_rw'@'<host>'` (idempotent via `IF EXISTS`; surgical/zero-gap — only DDL is named, so the running app's DML is never interrupted), leaving `SELECT,INSERT,UPDATE,DELETE` + the audit `INSERT`. The `app-rw` golden in [`verify-db-grants.ts`](../scripts/verify-db-grants.ts) is tightened to the DML set in the same change (req 6 paired edit), so the verify now FAILS the deploy if any DDL lingers on `app_rw`. Delete is a no-op (a teardown must not re-widen `app_rw`).
