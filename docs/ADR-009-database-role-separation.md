...
> **Implementation status (2026-05-30):** Phases 0–1 **landed** as code (the live effects are deploy-time events). Phases 2–3 pending; each is deploy-gated by the prior per the sequencing rule below.
...
- **Phase 2 — cutover.** Point the `sps-migrate` task `DATABASE_URL` at the migrate secret (one line in `app-stack.ts`); deploy; run a no-op migration to confirm `sps_migrate` works end-to-end. `app_rw` still holds DDL, now unused by migrate.
