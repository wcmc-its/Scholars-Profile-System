...
Search relevance is spread across several documents and **no single one covers it**. Reach for the
right one; a change argued in the wrong document tends to get reviewed against the wrong rules.

| Question | Document |
|---|---|
| What is a ranking term **allowed** to do? Which rules are currently broken? | **this document** — normative, plus the open-violation register |
| How does a People result actually get its score, layer by layer? | [`search-people-relevance.md`](./search-people-relevance.md) — descriptive |
| Why is a query returning too **few** people at all? | [`search-recall.md`](./search-recall.md) — admission, not ordering |
| How does a query string become a MeSH descriptor? | [`search-mesh-resolution-fallback-spec.md`](./search-mesh-resolution-fallback-spec.md), [`taxonomy-aware-search.md`](./taxonomy-aware-search.md) |
| What may the card **assert** about why someone matched? | [`search-evidence-rows.md`](./search-evidence-rows.md), [`scholar-card-evidence-rows-spec.md`](./scholar-card-evidence-rows-spec.md) |
| How do research-area pages rank scholars? | [`search-research-area-relevance-spec.md`](./search-research-area-relevance-spec.md) |
| Does publication age affect ordering? | [`search-recency-relevance-spec.md`](./search-recency-relevance-spec.md) |
| The Publications tab | [`search-publications.md`](./search-publications.md) |
| **Why** was a particular design chosen over the alternative? | the ADRs — currently [`ADR-011`](./ADR-011-concept-magnitude-ceiling.md) for the concept magnitude in People ranking |

⚠ **ADR-011 covers one term, not search relevance as a whole.** It governs the concept-magnitude
weight in the People prominence sum and nothing else. Its measurements (the census diff, the
`AREA_BOOST_TOP_N` convergence sweep, the query-log truncation rate) were taken on that term and do
not transfer to the method-family tier, the recall gates, resolution, or the Publications arm.
...
| O2   | Concentration boost credits area membership via a bare `terms: {cwid}` filter                          | Fixed behind `SEARCH_PEOPLE_CONCEPT_ARM_FIRST` — **on in both envs**; prod flipped 2026-07-30 (#2018/#2079). The descriptor-keyed arm is tried first, but its magnitude is still quantized to three bands — see O8 |
...
| E1b  | `topic` evidence line renders an index-time area total as query evidence                               | **Fixed** (#2071) — `areaCounts` is now resolved query-time (`areaCountsByCwid` in `searchPeople`, `lib/api/search.ts`), one batched `publicationTopic.groupBy` over the page cwids scoped to the topics the query resolved to. No new denominator: the line still states a magnitude, no share (E2b below is untouched). Sibling `#1367` (an on-topic count on the *clinical* line) is separate and not addressed here. |
...
E1b was untouched by this change (that 21 was still the scholar's index-time total in the area
whatever you searched) and is now closed separately — see the register row above.
...
- `docs/ADR-011-concept-magnitude-ceiling.md` — the decision to raise the concept-magnitude
