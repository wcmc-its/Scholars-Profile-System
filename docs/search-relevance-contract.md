...
| O8   | Every query-derived ordering term is a presence test; the only continuous term is career-total volume. A scholar with 318 publications tagged to the queried descriptor ranks 15th while one with 13 ranks first | Open — the root cause O1/O3/E1b are each a facet of. A per-publication, recency-aware scorer exists (`scorePublication`, `lib/ranking.ts`) and proves the shape is expressible here, but it is **not a drop-in**: it sums over `PublicationTopic` rows, which cover 4.9% of authored pmids with a median per-scholar ratio of **0**, and it weights each paper by impact, authorship and type rather than counting it once. Whatever fills O8 must cover the corpus the query actually ranks over |
