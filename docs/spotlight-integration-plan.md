...
**Amendment (2026-06-10, ReciterAI 25-card bump):** the spotlight artifact now publishes **every cleared candidate — up to 25 cards** (one per parent topic), instead of a pre-truncated top-9; the co-published schema's `spotlights.maxItems` was raised 10→25. No SPS schema/ETL change was needed — the ETL validates against the version-pinned co-published schema, so the producer-side `maxItems` bump is sufficient — and `getSpotlights()` was already uncapped. The on-page selection (`randomSample` → 8 of N) is unchanged, but now also **de-dups near-identical cards**: a couple of parent topics are containment-nested near-duplicates the producer's clone gate exempts (e.g. *Cancer Genomics & Molecular Oncology* vs *Genomics & Multi-Omic Profiling*, ~78% shared papers). At top-9 these rarely co-occurred; with all 25 shipping, a random-8 can draw both, which reads as a duplicate. `randomSample` now re-rolls (up to `MAX_CARD_REDRAWS`) when two drawn cards share ≥ `CARD_OVERLAP_THRESHOLD` of their displayed `papers[].pmid` (min-cardinality coefficient) — the card-level analogue of the existing `hasKeyAuthorCollision` paper re-roll. Stale "10"/`SPOTLIGHT_TARGET = 10` references in `etl/spotlight/index.ts` and `lib/api/home.ts` were corrected to 25. The "Render 10 …" / "one per parent topic" / "2–3 papers" wording below is the original v1 contract and is superseded on count. See [`spotlight-runbook.md`](spotlight-runbook.md) §Render rules and §Key constants.
