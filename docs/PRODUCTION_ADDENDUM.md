...
- **`nightly`**: `ed` → `reciter` → `asms` → `infoed` → `coi` → `dynamodb` → `mesh-coverage` → `search-index` → `revalidate`.
- **`weekly`**: `completeness` → `spotlight` → `reporter` → `nsf` → `jenzabar` → `search-index` → `revalidate`.
- **`annual`**: `hierarchy` → manual approval gate.

The authoritative step lists are the `nightlySteps` / `weeklySteps` / `annualSteps` arrays in [`cdk/lib/etl-stack.ts`](../cdk/lib/etl-stack.ts) — reconcile this section (and the cadence tables in `dependency-outage-matrix.md` / `architecture-overview.md`) against that file whenever the schedule changes.
...
aws stepfunctions start-execution \
  --state-machine-arn arn:aws:states:...:stateMachine:scholars-nightly \
  --input '{}'
```
...
```jsonc
// abbreviated ASL — the real definition (and the PascalCase `$.startFrom`
// tokens: "Reciter", "Dynamodb", "SearchIndexNightly", ...) is generated by
// `buildStateMachine` in cdk/lib/etl-stack.ts. This shows the nightly chain.
"StartAt": "Router",
"States": {
  "Router": {
    "Type": "Choice",
    "Choices": [
      { "Variable": "$.startFrom", "StringEquals": "Reciter",  "Next": "reciter" },
      { "Variable": "$.startFrom", "StringEquals": "Dynamodb", "Next": "dynamodb" }
    ],
    "Default": "ed"
  },
  "ed":           { "Type": "Task", /* ... */ "Next": "reciter" },
  "reciter":      { "Type": "Task", /* ... */ "Next": "asms" },
  /* asms -> infoed -> coi -> */
  "dynamodb":     { "Type": "Task", /* ... */ "Next": "mesh-coverage" },
  /* mesh-coverage -> */
...
With the router in place, `--input '{"startFrom": "Dynamodb"}'` actually resumes at the `dynamodb` step (skipping `ed`/`reciter`/`asms`/`infoed`/`coi`). Without it, the input is silently ignored and the state machine starts from the beginning anyway. Operators do not invoke individual steps directly under any circumstances.
...
- **Status alarm** (`sps-etl-<cadence>-status-${env}`) — metric math `ExecutionsFailed + ExecutionsTimedOut + ExecutionsAborted` (each an `AWS/States` `Sum`) `> 0` per state machine. Catches every execution that ended **without succeeding**, not just the ones that threw: a machine killed at its own `timeout:` ends as `TIMED_OUT` and runs no `Catch`, so it publishes nothing and never moves `ExecutionsFailed`; an operator `StopExecution` ends as `ABORTED`. Also catches crashes before the per-step `Catch` block runs. Evaluation period is 1 hour for the nightly / weekly / annual cadences; the **heartbeat overrides it to 24 hours** so one continuous stale-source condition yields one notification instead of one per day.
