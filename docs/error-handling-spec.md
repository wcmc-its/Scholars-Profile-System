...
| Slug redirects | `/[slug]` root-alias and `/scholars/[slug]` both resolve via `resolveBySlugOrHistory` → 301/308 for renamed slugs, `notFound()` otherwise. Since #2268 the resolver also carves the #536 hidden identity classes (and `/scholars/by-cwid/[cwid]` via `resolveByCwidOrAlias`), so a hidden scholar 404s exactly like a slug that never existed — the redirect can no longer publish the name in a `Location` header. | None. |
...
- **Adoption is incremental.** P5 ships the helper + the standard and adopts it in the operational routes where it is verifiably safe (`/api/revalidate`, `/api/analytics`). Routes already returning `{ ok: false, error }` (e.g. `/api/directory/people`, `/api/feedback/submit`) keep that variant; the broader retrofit is follow-up and must update each route's client in lockstep. (P5 also adopted it in `/api/health/refresh-status`, retired in #1514.)
- **Retrofit status (#686).** The deferred follow-up retrofit (tracked in #686) adopts `apiError` in the remaining hand-rolling routes:
  - **Adopted:** `scholars/[cwid]`, `scholars/[cwid]/popover-context`, `topics/[slug]/publications`, `topics/[slug]/subtopics/[subtopicId]/scholars`, `search` (invalid-topic), `publications/[pmid]`, `publications/[pmid]/citations.csv`, `export/publications/[granularity]`, `nih-portfolio` — all already returned a flat `{ error }`, so the swap is body-identical and only adds the `no-store` invariant. Plus `feedback/submit`, folded from `{ ok: false, error }` → `{ error }` (its client reads `res.ok` + `respBody.error`, so the change is transparent and adds the previously-missing `no-store`).
  - **Intentionally deferred / excluded:** `nih-resolve` returns `{ results: [] }` + 400 (a graceful-degradation contract the `GrantsSection` client reads as `.results` — *not* an error body; folding it would break the client). `directory/people` already emits `no-store` on every path; folding its `{ ok: false, error }` would touch three `/edit/*` unit-curation clients for no header gain. `impersonation/candidates` + `impersonation/route` are the active "View as" (#637) surface and a security gate — left untouched to avoid a parallel-branch collision. `auth/*` keep their redirect behavior (above). The `/edit/*` routes use their own `editOk`/`editError` helpers (already standardized within that subsystem). `csp-report` intentionally returns bodyless `204/413/415`. `health/*` carry the readiness-probe success shape, not an error body.
...
| **P5** ◑ | `lib/api/error-response.ts` `apiError` helper (flat shape, no-store) + tests + initial adoption (`revalidate`/`analytics`) | App deploy. **Full route retrofit = follow-up** (needs per-route client updates). |
