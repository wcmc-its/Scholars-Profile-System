...
| [`edit-console-ia-spec.md`](./edit-console-ia-spec.md) | Once inside `/edit`, which of the 17 console tabs each role actually sees. Three parts: (A) normative invariants (location-independence, nav/page parity, monotonicity) plus an intended role × tab matrix, executable as `lib/edit/console-tabs.server.ts` + `tests/unit/console-tab-matrix.{fixture,test}.ts` (proposed, not yet wired); (B) how the shipped `deriveConsoleTabs` baseline / per-page-override mechanism works today, and the Reports tab's own 1a/2a/3a sub-routing; (C) five confirmed gaps re-expressed as invariant violations (a pure `honors_curator` landing on `/edit` can lose the whole nav strip; the News tab can 404; Administrators is unreachable for a non-superuser unit Owner despite D5 requiring it; `/edit/units` doesn't surface Usage or Reports, and any comms_steward loses Units on `/edit/administrators`; a superuser with zero reportable centers 404s on a visible tab). |
...
| [`clinical-trials-source-spec.md`](./clinical-trials-source-spec.md) | Where a scholar's **Clinical trials** section comes from — `reciterdb.clinical_trials` (the pre-linked CWID spine, loaded by hand from a PI-only OnCore export since 2026-09-24) enriched live from the ClinicalTrials.gov v2 API on the nullable `nctNumber` (`clinical_trials_enriched` is only a fallback), why `protocolNumber` and not NCT is the natural key, why every link is "Principal Investigator", the inactive-trial CTA date rule, the Active/Completed display rule (Withdrawn and Suspended hidden, no year cut), and the weekly `etl:clinical-trials` refresh. |
...
| [`media-highlights.md`](./media-highlights.md) | Where a profile's **Media highlights** (press clips) come from and who approves them — the External Affairs digest → SES inbound (`Sps-InboundMail`, one bucket both envs read) → nightly `ClipsNightly` (`etl/news/clips.ts`) → pending `news_mention` rows with `outlet` set → `/edit/media-highlights-queue` → the profile section behind `MEDIA_HIGHLIGHTS_SECTION`; the funding digest sharing the mailbox (prod-only submit to ReciterAI); how to check and re-run a clip run. |
...
| [`title-hierarchy.md`](./title-hierarchy.md) | **Why a scholar shows *this* title under their name** — the External Affairs rank ladder (Dean 1 through Anything else 13, Vice Chair 8.5), the five candidate sources (working, ED primary, appointment, center director, division chief), and the rules: pins always win, ties go to the working title, emeritus holds no office, tracked centers rank as institutional, co-/associate directors never title, a director title naming its own department ranks as Chair. Reviewed in the Titles queue (`/edit/titles-queue`); a test pins the table to `TITLE_RANK`. |
...
| [`mentored-publications-report.md`](./mentored-publications-report.md) | **Where each pair on `/edit/reports/7` (Mentored publications) comes from, who may see it, and why a count is blank rather than 0** — the seven "Type of mentorship" sources (AOC pairing sheet incl. the MD-PhD program office and ECR, Jenzabar thesis advisors, ED postdoc supervisors, the two co-authorship inference tiers, each with what it carries and lacks), the per-person `report_access` grants and the Viewers panel, the `mtype=` (legacy `types=`) / `years=` / `tail=` / `pubs=` query contract plus the rail's post-load filters (`window` / `position` / `pubyear` / `mentor` / `withpubs`, applied to page and download alike), the 2026-09-24 page layout, and the rounds 3–6 rules (window = entry ≤ pub year ≤ grad + tail; the grad−4 entry fallback for AOC rows only; null in-window counts for year-less learners; iCite-only citations; the surest-source-wins rule; the "selected sources only" ceiling). Names the Faculty Review Tool extract as not yet a source. |
| [`reports-access-and-high-impact-publications.md`](./reports-access-and-high-impact-publications.md) | **Who can open which `/edit/reports` report** (unit, admin and per-person `report_access` gates; granting from "Who can run this report") and **what `/edit/reports/9` (Top clinical and high-impact journal publications) counts** — the journal families matched on `journal_abbrev`, the awards defaults, the rail (calendar or fiscal years), the Scholars and Publications tabs, and why the `.xlsx` People sheet is withheld above 50 people (`SCHOLAR_EXPORT_CAP`). |
...
| [`core-facility-claim-queue-spec.md`](./core-facility-claim-queue-spec.md) | **How a publication gets credited to one of WCM's 13 shared core facilities, and who confirms it** — ReciterAI's `pipeline_cores` engine candidates (acknowledgment / co-author / LLM-triage / repeat-user-prior signals) vs. the human `core_claim` override at the owner review queue `/edit/core/[coreId]`, the engine-status × claim merge (`lib/api/core-merge.ts`) that decides the effective status, and the public `/cores` + `/cores/[coreId]` surfaces and publication-detail-modal section — each gated behind its own flag (`CORE_PUB_MODAL`, `CORE_PAGES`, `CORE_CLAIM_WRITEBACK`), none yet deployed live. |
