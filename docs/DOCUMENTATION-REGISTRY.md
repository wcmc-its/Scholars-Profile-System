...
**Purpose.** This is the index to reach for _after_ launch, when someone — often an ITS
colleague — asks a question about how the Scholars Profile System (SPS) behaves in
production: _"Why is this page slow?"_, _"How is it deployed?"_, _"Where does this data
come from?"_, _"Is it down, or just degraded?"_, _"How do you authenticate edits?"_
...
`slug-personalization-*`, `unit-curation-*`, etc.). Those describe _what we are building_;
...
that still needs to be _collected_ into those docs, not docs that need writing — are tracked
in **§10**.

> Maintenance: when you add a doc that answers a _post-launch / operational_ question,
...
| Doc                                                              | Answers                                                                                                                                                                                             |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`performance-baseline.md`](./performance-baseline.md)           | The measured side of latency: per-surface baseline, alarm thresholds, and how to (re)measure. The counterpart to the SLO _targets_ in `SLOs.md`.                                                    |
...
_"why did you choose X over Y?"_
...
is **data to collect**, not docs to write — each item below is an open `TODO`/`TBD` _inside_
...
Distinct from §10 (which is _data to collect_), these are **decisions and confirmations
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- |
| 1   | **End-user latency SLO** — the current `p99 < 1.5 s` is the _origin_ tail. What is the target for CloudFront-_edge_-perceived latency?                                                              | Finalizes [`performance-baseline.md`](./performance-baseline.md) + the [`SLOs.md`](./SLOs.md) latency target.                                                          | SLO review (post-EdgeStack traffic)   |
...
_Last updated: 2026-06-03 — §0/§8 added [`search-recall.md`](./search-recall.md): why `covid19`/`tylenol`
...
follow-ons; §11 tracks open questions (see issue #560)._
