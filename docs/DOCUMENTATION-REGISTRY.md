...
| [`edit-console-ia-spec.md`](./edit-console-ia-spec.md) | Once inside `/edit`, which of the 17 console tabs each role actually sees — the `deriveConsoleTabs` baseline / per-page-override mechanism, the role × tab matrix, the Reports tab's own 1a/2a/3a sub-routing, and four confirmed gaps (a pure `honors_curator` landing on `/edit` can lose the whole nav strip; the News tab can 404; Administrators is unreachable for a non-superuser unit Owner despite the spec requiring it; `/edit/units` doesn't surface Usage or Reports). |
