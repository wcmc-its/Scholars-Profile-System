...
| `etl-page-${env}` (ETL, abort-tier) | **P1** | primary Teams channel |

Discrimination is on the topic-ARN substring (`:sps-warn-`, `:etl-failures-`) in `lambda/oncall-relay/index.ts` (`severityForRecord`); everything else is P1 -- `etl-page-${env}` has no substring match, so it falls through to the same P1 default as `sps-alarms-${env}`. The point is to keep data-freshness, reconciler, and resource-pressure signals off the on-call channel -- the core alert-fatigue fix -- so the page channel carries only customer-facing P1. P2/warn ALARM cards lead with a warning glyph (vs the alarm glyph on P1). Each card carries a **Severity** fact (`P1 (page)` / `P2 (warn)`), the alarm **Description**, and a **View reliability dashboard** button alongside the CloudWatch link. The Description is the single source of the what / why / what-to-do context: every alarm definition (`observability-stack.ts`, `etl-stack.ts`) ends its `alarmDescription` with a `Next:` first-response hint, so the guidance lives with the alarm (one place, edited in CDK) and reaches Teams verbatim -- the relay does not maintain a parallel hint table. The relay log line (`"event":"oncall_relay"`) records `severity` and the `channel` it actually delivered to.

`etl-page-${env}` is created in `cdk/lib/etl-stack.ts` (`this.pageTopic`, PR-7) for abort-tier step failures -- distinct from the P2 `etl-failures-${env}` topic above, which continue-tier steps and every status/cadence/duration alarm use. It is subscribed to the identical relay Lambda as the other three topics (`EtlPageRelaySubscription` in `observability-stack.ts`, wired the same way as `EtlFailuresRelaySubscription`), so delivery is live, not aspirational. What is NOT live yet: no CloudWatch alarm's `SnsAction` targets it -- `etl-stack.ts` has exactly one `SnsAction` and it points at `failureTopic` -- so today `etl-page-${env}` only receives the Step Functions `Catch` -> `SnsPublish` for steps declared `tier: "abort"` (currently `Ed`, `Dynamodb`, `SearchIndexNightly`, `IntegrityNightly`, `SearchIndexWeekly`, `IntegrityWeekly`).
...
5. **Fire a test alarm:**
   ...
   ```
   Expected within ~60 seconds: an Adaptive Card in the Teams channel rendering `🚨 sps-alb-5xx-rate-${ENV}`, state `ALARM`, the dry-run reason, the region, the timestamp, and a `View in CloudWatch` action button. **There is no recovery (`✅`) card.** The alarms set only `AlarmActions` (the page topic), not `OKActions`, so the return to `OK` is silent by design — verified 2026-06-14, `OKActions: []` on every `sps-*` alarm. Confirm recovery in the console/dashboard, not by waiting for a second card; if a resolve notification is ever wanted, add `addOkAction(alarmTopic)` to the alarms in [`../cdk/lib/observability-stack.ts`](../cdk/lib/observability-stack.ts). **A `status:202`/`outcome:"delivered"` line in the relay Lambda log is *not* proof a card rendered** — Power Automate returns 202 from the trigger *before* the post-to-channel step runs, so if the channel is empty the failure is inside the workflow (see § Diagnostics → Workflow Run history), not in AWS. If the card doesn't arrive within 5 minutes, see § Diagnostics; the Lambda's `OncallRelayErrors` alarm should also fire to the notify topic (email) if the Lambda itself errored.
...
- **Heavy path (AWS-side, B27):** The Teams subscription is now a CDK-managed Lambda, so `aws sns unsubscribe` is awkward (re-deploy would put it back). Prefer setting the staging Lambda's reserved concurrency to 0 (`aws lambda put-function-concurrency --function-name sps-oncall-relay-staging --reserved-concurrent-executions 0`) -- this drops every incoming SNS invocation to a `Throttle`. Restore with `aws lambda delete-function-concurrency --function-name sps-oncall-relay-staging`. Leaves a clear AWS-side audit trail and doesn't affect prod. **Since #2302, `OncallRelayErrors` watches Throttles as well as Errors**, so muting this way now DOES trip that alarm -- routed to the notify topic (email), not Teams, so it stays a quiet one-time heads-up rather than a page; expect exactly one such email when you flip concurrency to 0 and one more when you restore it.
