...
1. **Pre-deploy (one-time per env, in Teams):**
   ...
   - **Before capturing the URL, clear the request-body JSON schema on the manual trigger.** Open the workflow in Power Automate, expand the **When a Teams webhook request is received** (manual `Request`) trigger, and delete any contents from the request-body JSON schema field so it is empty. The default template ships with a JSON schema configured, which makes the trigger reject SNS's `application/x-www-form-urlencoded` SubscriptionConfirmation POST with HTTP 400 and silently drops the subscription (see § Gotchas: *SNS posts as form-urlencoded, not JSON*). Save the workflow after clearing the schema.
...
- **SNS posts as `application/x-www-form-urlencoded`, not JSON.** Per the [SNS HTTP/S delivery spec](https://docs.aws.amazon.com/sns/latest/dg/sns-message-and-json-formats.html#http-header), SNS sends the `SubscriptionConfirmation` (and subsequent `Notification`) POSTs with `Content-Type: application/x-www-form-urlencoded; charset=utf-8` even though the body itself is JSON. The Power Automate "Post to a channel when a webhook request is received" template configures its manual `Request` trigger with a JSON request-body schema by default; that trigger rejects form-urlencoded with `HTTP 400 InvalidRequestContent: The input body for trigger 'manual' of type 'Request' must be of type JSON`. SNS treats the 400 as a failed endpoint validation and silently does not create the subscription -- `aws sns list-subscriptions-by-topic` returns `[]` after a failed attempt, with nothing in the channel and no AWS-side error. Fix it on the Workflow side by clearing the request-body JSON schema field on the manual trigger (the trigger then accepts arbitrary bodies); alternatives are a pre-step that re-encodes form data to JSON, or replacing the manual trigger with a webhook-style trigger. Reproduced 2026-05-21 wiring `sps-alarms-staging` to the staging Teams channel; see #434. Avoid the trap by following the schema-clearing sub-step in § Rollout per env above; the test path that surfaces the trap is `aws sns subscribe ... --protocol https --endpoint <workflow-url>` returning an XML-parse error referencing `InvalidRequestContent`.
