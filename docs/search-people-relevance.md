...
scholar can be HI in one area and LO in another with identical work. And `AREA_BOOST_TOP_N = 200` is a
cliff — in arm 2 the 200 candidates come from a `terms` agg ordered by `doc_count` (`:1502`), i.e. they
are the highest-**volume** on-topic authors, reranked by concentration only afterwards. A genuinely
concentrated low-volume specialist outside the top 200 by raw count gets exactly 0.
