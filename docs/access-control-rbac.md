...
| **Deploy pipeline broken, must ship out-of-band** | Operator runs the 5-step pipeline manually with their AWS creds — migration task **must** exit 0 before the service rolls (ordering is load-bearing). | [`DEPLOY-RUNBOOK.md § Five-step pipeline contract`](./DEPLOY-RUNBOOK.md) |
