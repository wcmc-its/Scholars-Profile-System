...
| **Bad image, 5xx, circuit-breaker didn't catch it** | **Operator-driven rollback:** `ecs update-service --task-definition <FAMILY>:<PREVIOUS_REVISION>` — a real, distinct, digest-pinned revision (#2121). Do **not** re-run the workflow from an old commit (re-runs migrations). | [`DEPLOY-RUNBOOK.md § Emergency procedures`](./DEPLOY-RUNBOOK.md) |
