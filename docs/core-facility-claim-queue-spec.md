...
### Manual PMID add — a goal, extending this same endpoint (not yet built)

An owner who knows a paper used their core — one the engine never scored, or scored `below_threshold` — has no way to say so today. The ask: let an owner **paste a block of known PMIDs** and claim them directly, independent of the engine queue.

**Write path needs one addition, not a new endpoint.** This bulk route already accepts an arbitrary `pmids: string[]` with no `publication_core`/`publication` existence check — it validates only pmid format, `core` existence, and authz — so a pmid with no engine row already upserts a `core_claim` row successfully today (`core_claim` is deliberately FK-less, see Data model). The gap: nothing tells the caller a pmid was bogus, it just silently writes a claim that then displays nowhere (see below) — a real footgun for a paste-a-list UI. Close it with one new check mirroring the existing `core.findUnique` existence-check already in this route: `db.read.publication.findMany({ where: { pmid: { in: pmids } }, select: { pmid: true } })`, returning any pmid not in that set as a new `notFound` list in the response, alongside the existing `skipped`-already-claimed count, instead of writing it. Everything else — the transaction, the audit row, `MAX_BULK_PMIDS = 500` — is unchanged. `revoked` stays out of scope here too (single-row-only, unchanged) — manual add only ever writes `claimed`; rejecting a pmid the engine never saw isn't a meaningful action.

**Read path is the real work — one more query per surface, copying a pattern already in this codebase.** All three consumers (`loadCoreReviewQueue`/`lib/api/core-queue.ts`, `getCorePage`/`lib/api/cores.ts`, `resolvePublicationCores`/`lib/api/publication-detail.ts`) start from `publicationCore.findMany` and overlay `core_claim` on top by key — a claim with no matching `publication_core` row loads into the overlay map but is never iterated as a display row, so it's dead data today. `core-merge.ts`'s helpers need no changes for this: `effectiveCoreStatus`/`isEffectiveConfirmed` both return as soon as an active claim exists, before ever reading the engine status. The fix at each of the three call sites is a fourth, independent query for `core_claim` rows with no matching `publication_core` row, joined directly to `Publication` (and `Core`, for the modal) for display fields, unioned into the same by-key collection before handoff to the existing partition/select functions — the exact shape `getMenteesForMentor` (`lib/api/mentoring.ts`) already uses to fold `getManualMentees` (`lib/api/manual-layer.ts`, a purely additive human-only source with no engine row) in alongside three engine-sourced queries, degrading to `[]` on failure rather than failing the whole section. Copy that shape, substituting `claimKey(pmid, coreId)` for `cwid`.

**UI.** One "Add PMIDs" affordance in the owner queue header, next to "Confirm N high-confidence" — a textarea taking a newline/comma-separated block, posting the parsed list to the (now-widened) bulk endpoint with `status: "claimed"`. Result surfaces per-pmid: added / already-claimed / not-found-in-SPS, mirroring the endpoint's existing `skipped`-count pattern. A manually-added row's card won't break — `buildSignals()` already renders a 0-signal row gracefully ("0 of 4 signals fired", no evidence list). One open display question: the likelihood bar has no natural value for a manual claim and would misleadingly show 0% next to a confident human decision — needs a small carve (hide the bar, or a "Manually added" badge), a product call rather than a functional blocker.
...
- **No way to manually claim a PMID the engine hasn't scored at all** — the queue UI has no free-text/paste PMID entry, and while nothing at the write layer would reject such a claim (`core_claim` is FK-less, see Data model), every read path is `publication_core`-first, so it would surface nowhere. SPEC'd above ("Manual PMID add") as an extension of the existing bulk endpoint plus a fourth union query at each of the three read paths, copying the `getManualMentees` pattern; not yet built.
