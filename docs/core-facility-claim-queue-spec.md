...
- **No way to manually claim a PMID the engine hasn't scored at all.** The queue UI has no free-text PMID entry, and `POST /api/edit/core-claim` doesn't check for a matching `publication_core` row before writing — only that the `core` itself exists — so nothing at the API layer would reject such a claim. But it wouldn't *work*: every read path (owner queue, public core page, publication-detail modal) queries `publication_core` first and overlays the claim onto it, so a claim with no matching row is written durably to MySQL and then surfaces nowhere. `core_claim`'s FK-less design (see Data model) exists to let a claim precede the engine's projection for a pub the engine *will* eventually score, not to let a human attest usage the engine never will (e.g. a pre-pipeline paper). Supporting that for real needs a claim-first read path, not just a text box — a natural companion to the `below_threshold`-promotion gap above.
