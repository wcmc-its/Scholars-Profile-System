...
> **Amendment, 2026-09-22 (#2719).** PE-03's hide set was originally `publication` alone. That was narrower than the principle D4 states — *"the proxy is acting for the scholar, so they get the scholar's surface, no more"* — because the scholar's own surface **already** includes hiding their own appointment, education and grant rows (`authorizeSuppress`'s whole-entity branch). A proxy or unit admin who tried got a `403` that could never succeed, on a row the scholar could hide unaided. The three whole-entity types are therefore added to the delegated hide, restoring D4's stated intent rather than widening past it.
>
