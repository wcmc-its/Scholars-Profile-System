...
- **What is α actually?** ≈ 0.5 is inferred from two independent signals, neither of which swept α directly. The three-dimensional sweep measures it.
  ...
  🔴 **Panel sweep done (#2097), not the answer — the census diff and blind panel are still open.** 180 curls, 10 panel queries × α∈{0,0.5,1} × `W_HI`∈{3,12,20} × `dampen`∈{off,capped}, ship-config posture pinned, flag echo asserted on every capture (PR #2108 unblocked this). Findings:

  - **`dampen` interacts broadly, confirming B3's thesis rather than just asserting it**: 89 of 90 (α, `W_HI`) combos show a different top-10 between `dampen=off` and `dampen=capped` — it touches nearly every query, not just concept-heavy ones, because it caps the *outer* prominence sum's volume term. Tuning `W_HI` against one `dampen` posture and shipping the other would have invalidated the result, exactly as feared above.
  - **`W_HI` knee reconfirms the earlier finding**: flat between 12 and 20 for 8 of 10 panel queries at α=1; `cancer` and `aging` keep creeping past 20, consistent with `cancer`'s known-truncated pool (#2096) rather than a real signal.
  - **α=0.5 is a gentle step from today's α=1** (8–10/10 top-10 overlap on 8 of 10 queries); **α=0 is not** — it opens a real gap specifically on the two broadest, most generalist-heavy queries (`cancer`: 5/10 overlap, `aging`: 6/10), which is exactly where the ADR predicted a prolific generalist would start winning. This corroborates α ≈ 0.5 as the region to search, not the extremes — but it is still a panel-only signal (n=10), not a swept α value with its own acceptance evidence.

  **Still open, unchanged by this pass**: the full 313-query census diff at the chosen `(α*, W*, dampen*)` (the ADR's own "primary acceptance evidence," which the panel is not a substitute for), and the blind judged panel itself — real relevance judgment needs a human judge and is not something to simulate. Full data and method in #2097; not reproduced here.
