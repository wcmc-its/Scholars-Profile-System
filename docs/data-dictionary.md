...
| **`ctsc_feed_issue`** (`CtscFeedIssue`) | Internal (CTSC feed vs ED) | One CTSC feed record whose CWID CTSC should fix at the source. PK `primaryKey` (feed PrimaryKey); `name`, `institution`, `feedCwid`, `reason` (`blank-resolved`, `not-in-ed`, `retired-cwid`, `cwid-email-conflict`, `email-match-name-differs`, `email-ambiguous`, `duplicate-record`), `suggestedCwid` / `suggestedName` / `matchedEmail` (what ED resolved from the record's emails, null when nothing resolved unambiguously), `syncedAt`. Full-replaced nightly by `etl/ctsc-roster`; shown as "Feed CWID issues" on the CTSC center's /edit page. |
