...
4. **`Scholar.overviewUpdatedAt`.** An orphaned column — no application code writes it or reads it, and `field_override.updatedAt` / `overview_provenance.updatedAt` timestamp overview edits. **Recommendation: leave it orphaned in v1** (the profile does not render it); revisit only if an "updated" date is wanted on the profile.
   **Update (#2212, prod 2026-08-05): the column is not merely empty, it is misleading.** All 552 seeded rows carry the *identical* value `2026-05-06` — the one-time corpus-load date, written by the bulk data load (`scripts/interim-populate/`), not by any code path. It is a LOAD timestamp wearing a content timestamp's name. The upstream VIVO export carried no per-bio authored/modified date, so there is nothing to backfill it from and no way to make it track content age retroactively; going forward, an /edit save already records its own date in `overview_provenance`, so writing this column too would only duplicate that. Anything wanting "when was this bio last changed" must read `overview_provenance.updatedAt` and treat its absence as "never edited since import" — never this column. **Open decision for a human: drop the column outright** (a contract migration — forecloses the future profile "updated" date, and is irreversible on prod), **or keep it orphaned with this warning.** Until that is decided, do not wire a reader to it.
