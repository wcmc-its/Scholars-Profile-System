...
| NEWS_MENTIONS_SECTION | on | on | News mentions (docs/2026-07-18-news-mentions-plan.md) | Profile "News mentions" section + /edit card; presence-gated. Prod flipped 2026-07-27 once etl:news seeded (870 articles → 1,595 mentions, 1,112 renderable across 476 scholars) |
| NEWS_APPROVAL_QUEUE | on | on | News mentions (docs/2026-07-18-news-mentions-plan.md) | /edit/news-queue comms surface + decision endpoint; audience = superusers + external comms (comms_steward). Prod flipped 2026-07-27 alongside the section |
...
| 2026-08-01 (#1345, prod flip to OFF — converged the OTHER direction, not on-in-both) | SEARCH_PEOPLE_FACULTY_PROMINENCE. Re-eval (07-03, PR #1471) found turning it ON regresses overall relevance (0.292→0.285 meanMRR) now that #1363's concentration boost is active, and the diabetes-cluster regression that originally parked this issue does not reproduce with #1363 active. Both envs now `off`, verified in the deployed prod task def (`sps-app-prod:45`) |
| 2026-08-02 (#1972 resolution) | SEARCH_MESH_RESOLUTION_FALLBACK. Of the 3 originally-measured demotions on the 35-query free-typed set, `chronic fatigue` was fixed by #1975, `breast cancer screening` by #1979/#2161's same-descriptor retry-upgrade (verified live: resolves `Breast Neoplasms`/`entry-term`), and `foreign policy` → `Policy` is an accepted, deliberately unfixed trap (its candidate fix, `SEARCH_MESH_RESOLVE_TOKEN_COVERAGE`, was tried, measured, and reverted — see that flag's own row in the dark-everywhere queue below). Matcha's ranking already weighted on this confidence tier internally; #2168 made the tier visible on the returned `MatchaConcept` contract. Both envs now `on`, verified in the deployed prod task def (`sps-app-prod:46`) |
| 2026-07-31 (#2068, ADR-011 B1/B3/B4, PR #2138) | SEARCH_PEOPLE_PUBCOUNT_DAMPEN. Was misfiled below as dark-everywhere pending a staging→prod flip gated on the FACULTY_PROMINENCE prereq; that prereq is moot because this shipped `capped` in **both** envs directly, ahead of and independent of the FACULTY_PROMINENCE convergence above. Verified in the deployed prod task def (`sps-app-prod:43`, registered 2026-07-31). The Matcha spine still pins its own copy `off` (unaffected — enabling it there needs a sponsor-gold re-baseline, and that gold is nDCG@20-graded) |
| 2026-08-17 (#1468) | REPORTER_MATCH_V2 (both app-stack and etl-stack copies). Staging match-quality spot-check signed off. Deploy verified 2026-08-31: `on` in the running task defs `sps-app-prod:88` and `sps-etl-prod:11`, and the prod weekly ETL shows the v2 matcher live (08-30 run: 18 auto-locked, 2 pending). #1468 closed |
...
1. **No cdk deploy — either env — without** `scripts/release/whats-shipping.sh <env>` + a `cdk diff Sps-App-<env>` review. A prod App deploy is a release of everything above, not your change; and staging is not exempt, because staging's origin leg is live traffic too (#1937).
   ...
   *Why this rule grew a staging half:* #1929 pinned the ALB `:443` TLS policy to `ELBSecurityPolicy-TLS13-1-2-Res-PQ-2025-09` and merged, but staging was never deployed, so staging sat on the permissive `...-PQ-2025-09` (4 CBC ciphers) for days — on what is actually its live origin leg. A synth-time assertion cannot catch this; it passes regardless of what is deployed. `cdk diff Sps-App-staging` reported it immediately. (Resolved 2026-07-27 — both envs are now at parity, verified by live handshake. The rule stays because the *class* of failure is chronic here, not because that one instance is open.) **Honest limit: this fires only when an operator runs it. Nothing in this repo runs on a schedule with AWS credentials, so there is no continuous detection between deploys.**
