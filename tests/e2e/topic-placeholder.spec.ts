 ...
 *   - When the Recent highlights section renders, it has the 'how this works'
 *     link wired to /about/methodology#recent-highlights AND no citation
 *     counts surface in the section.
 *
...
test.describe("/topics/{slug} placeholder route (Phase 2 D-10)", () => {
  ...
  test("Recent highlights section, when present, links to /about/methodology#recent-highlights and shows no citation counts", async ({
    page,
  }) => {
    for (const slug of CANDIDATE_SLUGS) {
      const resp = await page.goto(`/topics/${slug}`);
      ...
      if (await heading.isVisible().catch(() => false)) {
        ...
        await expect(link).toHaveAttribute(
          "href",
          "/about/methodology#recent-highlights",
        );
    ...
    test.skip(
      true,
      "Recent highlights section not visible on any candidate topic",
    );
  });
});
