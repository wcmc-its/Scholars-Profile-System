...
describe("classifyMenteeKind", () => {
  it("picks the most specific trainee type over the paid-student employee type", () => {
    expect(classifyMenteeKind(["employee", "employee-student-paid", "student", "student-phd-weill"])).toBe("doctoral");
    expect(classifyMenteeKind(["student", "student-md-phd-tri-i"])).toBe("md_phd");
    expect(classifyMenteeKind(["academic-nonfaculty", "academic-nonfaculty-postdoc-fellow"])).toBe("fellow");
    expect(classifyMenteeKind(["academic", "academic-nonfaculty", "employee"])).toBe("research_staff");
  });
