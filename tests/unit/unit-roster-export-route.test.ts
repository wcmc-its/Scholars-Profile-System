...
describe("/edit/center/[code]/export gating", () => {
  ...
  it("returns a CSV attachment with the roster header + rows for an authorized actor", async () => {
    ...
    expect(lines[0]).toBe(
      "cwid,name,title,membership_type,program_code,program_label,start_date,end_date,status,source,email,role_category,department,division,scholar_state,institution",
    );
