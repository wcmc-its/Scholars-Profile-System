/**
 * `lib/auth/cv-generator.ts` — the read-only cv-generator role.
 *
 * Mirrors tests/unit/data-sharing-viewer.test.ts, which covers the same shape
 * (kill switch + group-DN resolve + LDAP `compare`, no allowlist) for the
 * sibling role this one is modeled on most closely.
 *
 * Every case below is a fail-closed case. This role can unlock every
 * scholar's profile (read-only), so a directory problem must never *grant* it.
 */
...
import { isCvGenerator, isCvGeneratorEnabled } from "@/lib/auth/cv-generator";
import { openLdap } from "@/lib/sources/ldap";

// The real cn — see cdk/lib/app-stack.ts's SCHOLARS_CV_GENERATOR_GROUP_CN.
const GROUP_CN = "ITS:Library:Scholars/cv-generator-role";
...
beforeEach(() => {
  vi.resetAllMocks();
  process.env.CV_GENERATOR_ENABLED = "on";
  process.env.SCHOLARS_CV_GENERATOR_GROUP_CN = GROUP_CN;
  delete process.env.SCHOLARS_LDAP_SEARCH_BASE;
  vi.spyOn(console, "warn").mockImplementation(() => {});
});

describe("isCvGeneratorEnabled", () => {
  it("is true only when CV_GENERATOR_ENABLED is exactly 'on'", () => {
    expect(isCvGeneratorEnabled()).toBe(true);
    process.env.CV_GENERATOR_ENABLED = "off";
    expect(isCvGeneratorEnabled()).toBe(false);
    process.env.CV_GENERATOR_ENABLED = "ON";
    expect(isCvGeneratorEnabled()).toBe(false);
    delete process.env.CV_GENERATOR_ENABLED;
    expect(isCvGeneratorEnabled()).toBe(false);
  });
});

describe("isCvGenerator", () => {
  it("short-circuits to false (no LDAP) when the kill switch is off", async () => {
    process.env.CV_GENERATOR_ENABLED = "off";
    expect(await isCvGenerator("abc1234")).toBe(false);
  ...
  it("is true when the group lists the CWID as a member", async () => {
    ...
    expect(await isCvGenerator("abc1234")).toBe(true);
  });
  ...
  it("fails closed on a CWID that is not DN-safe, before any directory work", async () => {
    const client = fakeClient();
    mockedOpenLdap.mockResolvedValue(asClient(client));
    expect(await isCvGenerator("a*b)(uid=*")).toBe(false);
    expect(await isCvGenerator("abc1234,ou=people,dc=x")).toBe(false);
  ...
  it("is dormant, not broken, when the group cn is unset (e.g. before the ED group exists)", async () => {
    delete process.env.SCHOLARS_CV_GENERATOR_GROUP_CN;
    expect(await isCvGenerator("abc1234")).toBe(false);
    expect(mockedOpenLdap).not.toHaveBeenCalled();
  });

  it("is false and never touches LDAP for an empty CWID", async () => {
    expect(await isCvGenerator("")).toBe(false);
    expect(mockedOpenLdap).not.toHaveBeenCalled();
  });
