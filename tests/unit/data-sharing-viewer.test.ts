/**
 * `lib/auth/data-sharing-viewer.ts` — the data-sharing dashboard viewer role.
 *
 * Mirrors tests/unit/honors-curator.test.ts, which covers the same shape
 * (kill switch + group-DN resolve + LDAP `compare`, no allowlist) for the
 * sibling role this one is modeled on most closely.
 *
 * Every case below is a fail-closed case. This role can unlock a dashboard
 * with department/faculty rollups, so a directory problem must never *grant* it.
 */
...
import { isDataSharingViewer, isDataSharingViewerEnabled } from "@/lib/auth/data-sharing-viewer";
import { openLdap } from "@/lib/sources/ldap";

// The real cn — see cdk/lib/app-stack.ts's SCHOLARS_DATA_SHARING_VIEWER_GROUP_CN.
const GROUP_CN = "ITS:Library:Scholars/data-sharing-viewer-role";
...
beforeEach(() => {
  vi.resetAllMocks();
  process.env.DATA_SHARING_VIEWER_ENABLED = "on";
  process.env.SCHOLARS_DATA_SHARING_VIEWER_GROUP_CN = GROUP_CN;
  delete process.env.SCHOLARS_LDAP_SEARCH_BASE;
  vi.spyOn(console, "warn").mockImplementation(() => {});
});

describe("isDataSharingViewerEnabled", () => {
  it("is true only when DATA_SHARING_VIEWER_ENABLED is exactly 'on'", () => {
    expect(isDataSharingViewerEnabled()).toBe(true);
    process.env.DATA_SHARING_VIEWER_ENABLED = "off";
    expect(isDataSharingViewerEnabled()).toBe(false);
    process.env.DATA_SHARING_VIEWER_ENABLED = "ON";
    expect(isDataSharingViewerEnabled()).toBe(false);
    delete process.env.DATA_SHARING_VIEWER_ENABLED;
    expect(isDataSharingViewerEnabled()).toBe(false);
  });
});

describe("isDataSharingViewer", () => {
  it("short-circuits to false (no LDAP) when the kill switch is off", async () => {
    process.env.DATA_SHARING_VIEWER_ENABLED = "off";
    expect(await isDataSharingViewer("abc1234")).toBe(false);
  ...
  it("is true when the group lists the CWID as a member", async () => {
    ...
    expect(await isDataSharingViewer("abc1234")).toBe(true);
  });
  ...
  it("resolves the group DN by cn ALONE — no `member` predicate in the search", async () => {
    ...
    await isDataSharingViewer("abc1234");
    expect(client.search).toHaveBeenCalledWith(GROUPS_BASE, {
      scope: "sub",
      filter: `(cn=${GROUP_CN})`,
      attributes: ["cn"],
    });
    // Regression guard, same as every sibling role's: a `member` predicate here
  ...
  it("fails closed on a CWID that is not DN-safe, before any directory work", async () => {
    const client = fakeClient();
    mockedOpenLdap.mockResolvedValue(asClient(client));
    expect(await isDataSharingViewer("a*b)(uid=*")).toBe(false);
    expect(await isDataSharingViewer("abc1234,ou=people,dc=x")).toBe(false);
    expect(client.compare).not.toHaveBeenCalled();
  });

  it("is dormant, not broken, when the group cn is unset (e.g. before the ED group exists)", async () => {
    delete process.env.SCHOLARS_DATA_SHARING_VIEWER_GROUP_CN;
    expect(await isDataSharingViewer("abc1234")).toBe(false);
    expect(mockedOpenLdap).not.toHaveBeenCalled();
  });

  it("is false and never touches LDAP for an empty CWID", async () => {
    expect(await isDataSharingViewer("")).toBe(false);
    expect(mockedOpenLdap).not.toHaveBeenCalled();
  });

  it("fails closed when the group cn is not present in the directory (e.g. an empty group)", async () => {
