  ...
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    appTaskDefinition.addContainer("app", {
      ...
      environment: {
        ...
        // #1762 -- the /edit honors approval queue. Enabled in BOTH envs
        // (2026-07-18): prod flips as part of the #1761 seed rollout. The queue
        // is gated (superuser / honors_curator) and nothing it approves can
        // render until a human approves it, so with the seed landing the
        // empty-surface objection no longer applies.
        ...
        //     nothing. Flipped WITH the seed and the queue (2026-07-18).
        //     ⚠ The staging with-a-real-curator verification of the group's
        //     employee-only memberURL filter (below) is still outstanding; its
        //     failure mode is fail-CLOSED (curator sees no tab; superusers
        //     unaffected), so enabling ahead of that check risks nothing.
        ...
        // Staging soaked + reindexed 2026-08-07 (API + browser eyeball both
        // clean, incl. a live click-through: earlyStageInvestigator=true
        // correctly narrowed a 4,280-result query to 841 and recomputed every
        // other facet under it). Prod-on 2026-08-10, paired with a prod
        // reindex on the same day. Two independent kill switches on purpose:
        // SEARCH_PEOPLE_CLINICAL_RANK_FACETS is a low-risk direct-column
        // copy; SEARCH_PEOPLE_ESI_FACET is the riskier, novel derivation and
        // gets its own lever.
