...
_Last updated: 2026-04-29 (v1.6)_
...
| Doctoral students | Records under `ou=students` in Enterprise Directory (distinct from `ou=faculty`); identifies via `weillCornellEduDegreeCode = PHD` or similar | ~500 (TBD) |
...
The visible UI element that signals AI-driven selection on a surface.

```html
<span class="curated-tag">
  Curated
  <span class="curated-tag__info" title="Ranked by ReCiterAI weekly. Scoring covers
    publications co-authored by full-time WCM faculty; co-authors inherit visibility
    regardless of role.">i</span>
</span>
```

The freshness disclosure ("weekly") matters for the same credibility reason as the scope disclosure: a user who refreshes hourly and sees no change should understand why.

Visual: title-case "Curated" (not all-caps), slate-light background, slate text, small "i" info icon.
...
**Preamble.** The italic introduction paragraph above the categories is the WCM-authored framing language — committee-produced, institutional voice, deliberately preserved verbatim. Do not edit, paraphrase, or summarize. The paragraph is part of the institutional disclosure protocol, not flavor copy.
...
**Factor 3: Publication type weight.** Soft preference for original research, applied across all four surfaces:
...
| Preprint | 0.7 |
| Letter | 0.1 |
| Editorial Article | 0.1 |
| Erratum | 0.1 |

A transformative Review can still surface as a highlight, but must clear a substantially higher bar than an Academic Article of equivalent ReCiterAI score. Letters and Editorials effectively never surface as highlights but are not hard-excluded — a 10× score gap can still rescue them in edge cases. Preprints get the same weight as Reviews because in fast-moving fields (genomics, ML methods) a high-impact preprint can be the most consequential output.
...
**Owner: TBD.** Name an owner before the spec is circulated. Candidates: OFA (if treated as an institutional research-information artifact), ITS (if treated as product documentation), or shared between them with one editor as primary. **Decision needed by:** {date — fill before circulation}.
...
**Decision needed by:** {date — fill before search-build kickoff}.
...
**Decision needed by:** {date — fill before doctoral-student profile build}.
...
**Decision needed by:** {date — fill before profile build}.
...
19. **Division URLs route to parent department** in Phase 1. Visiting `/departments/{dept}/divisions/{div}` resolves to the parent department detail page with the division pre-selected in the rail. No 404s, no broken-looking redirects, and no dedicated division page in Phase 1.
