/**
 * Matcha — the UI ⇄ ranker CONTRACT.
 *
 * This file IS the seam. The ranker (`sponsor-match-spine-run.ts`) produces
 * `MatchaResponse`; the console (`components/edit/matcha-panel.tsx`)
 ...
 *
 * `MatchaCandidate.contributions[]` carries `rank_{s,c}` for EVERY concept the
 ...
 * via `matchaMeasuresFrom` below, so career stage, clinician status and person type are
 * real). `preferences` has one (`extractMatchaPreferences`). `ask` now has one too —
 * `askTitleFrom` below, DERIVED from the concepts rather than generated by an LLM. `facets`
 ...
 * `searchEvidence` — see `MatchaSearchEvidence`. `evidence` (the bespoke shape) remains for
 ...
 * the slice (see `MatchaCandidate.searchEvidence`), so a candidate whose two strongest concepts
 * are unresolved clusters still ships the three below them rather than a short list.
 */
export const MAX_EVIDENCE_CONCEPTS = 3;

export type MatchaFitTier = "strong" | "good" | "weak";
 ...
 * points back to.
 */
export type MatchaConcept = {
  /** Representative term. The join key for `MatchaCandidate.contributions[].term`. */
  term: string;
...
export type MatchaContribution = {
  /** Joins to `MatchaConcept.term`. */
  term: string;
 ...
 * the title string.
 */
export function matchaMeasuresFrom(row: MatchaMeasuresRow, now: Date = new Date()): MatchaMeasures {
  return {
 ...
 * it belongs to.
 */
export type MatchaSearchEvidence = {
  /** The concept this evidence is FOR. Joins to `MatchaContribution.term` and
   *  `MatchaConcept.term` — the same representative-term key everything else in this
...
export function preferenceBoost(
  ...
  preferences: readonly MatchaPreference[],
): number {
  let matched = 0;
  let total = 0;
...
export type MatchaResponse = {
  ok: true;
  concepts: MatchaConcept[];
  candidates: MatchaCandidate[];
  /** A short handle for this search — see `askTitleFrom`. Absent when the paste yielded
...
export type RerankOptions = {
  ...
  prefBoost?: (candidate: MatchaCandidate) => number;
 ...
 *  a comment. */
export function conceptWeight(concept: MatchaConcept): number {
  return concept.centrality ** CENTRALITY_GAMMA * concept.weightFactor;
}

/** Term → fusion weight, for a whole concept set. */
export function conceptWeights(concepts: readonly MatchaConcept[]): Map<string, number> {
...
export function matchedConcepts(
  ...
  concepts: readonly MatchaConcept[],
  opts: RerankOptions = {},
): { concept: MatchaConcept; strength: number }[] {
  const k = opts.k ?? DEFAULT_K;
 ...
 * `MAX_EVIDENCE_CONCEPTS` by strength at DEFAULT weights (see `MatchaCandidate.searchEvidence`
...
export function matchedEvidence(
  ...
  concepts: readonly MatchaConcept[],
  opts: RerankOptions = {},
): { concept: MatchaConcept; evidence: MatchaSearchEvidence }[] {
 ...
 */
export function fitTier(score: number, topScore: number): MatchaFitTier {
 ...
 */
export function rareTerms(concepts: readonly MatchaConcept[]): Set<string> {
  const known = concepts.filter(
    (c): c is MatchaConcept & { corpusCoverage: number } =>
 ...
 */
export function hasMatchEvidence(candidate: MatchaCandidate): boolean {
 ...
 *
 * NOT `MatchaSearchEvidence.pubCount`, which is the scholar's TOTAL (the "M") and is therefore the
 ...
 */
export function latestEvidenceYear(candidate: MatchaCandidate): number | null {
