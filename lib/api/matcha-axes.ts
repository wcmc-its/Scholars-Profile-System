 ...
 *
 * MEASURED AND REJECTED — do not re-propose without reading this. Picking the BROADEST member
 * (largest descriptor set) instead of the earliest was implemented behind `MATCHA_BROAD_REP`,
 * A/B'd over 10 paired draws on 2026-07-27, and removed. The result: mean paired difference
 * **−0.00011 nDCG@20** over graded candidates, 95% CI [−0.00016, −0.00006] — statistically real,
 * negative, and ~65× below the extractor's own ~0.0074 noise floor. Only 3 of 15 sponsor fixtures
 * were sensitive to the rule at all, and all of the movement came from one.
 *
 * So the ranking case was neutral, and the coherence case did not carry it: the broadest member
 * makes the union a subset of the rep's subtree ONLY for a subsumption merge. A Jaccard merge is
 * non-nested by definition, so no member's subtree contains the union and it buys nothing there —
 * the ground on which #1978 rejected it. #1978 already made the naming honest (the provenance set
 * is the rep's own subtree), leaving a label/count mismatch nobody has reported, against a blast
 * radius that is the cluster identity itself: the wire `concept.term`, the include chips and
 * `applyIncludes`, the culled tail, `kind` → `weightFactor`, and `keyPaper.conceptLabel`.
 *
 * Full write-up: `docs/2026-07-27-1977-cluster-representative-eval-findings.md`.
 */
