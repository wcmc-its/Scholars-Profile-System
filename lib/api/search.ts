...
export async function searchPeople(opts: {
  ...
  const prominenceFunctions: Record<string, unknown>[] = applyProminence
    ? [
        ...
        // The productivity prior (plain ln1p or the #2068 capped ladder, per
        // `pubCountFunctions` above), dropped entirely when `pubcountProminence` is off.
        // Paired with the inner step functions below — see `pubcountProminence`, which
        // gates both halves of the same double-priced signal.
        ...(opts.pubcountProminence !== false ? pubCountFunctions : []),
