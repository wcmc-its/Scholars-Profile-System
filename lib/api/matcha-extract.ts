...
const EXTRACT_SYSTEM_PROMPT = [
  ...
  "diseases, methods, mechanisms, and populations it targets — NOT organization names, award",
  "mechanics, dollar amounts, eligibility rules, or deadlines.",
  "",
  "Return each concept as a short, canonical noun phrase suitable for a MeSH / taxonomy",
  'lookup. Prefer the standard medical term over brand names or jargon:',
  ...
  "them at the top. Exactly ONE concept is the PRIMARY TARGET and scores 1.0: the thing the",
  "description is fundamentally about. It may be a disease, a population, or a method —",
  "whichever it is really seeking.",
  "",
  "Score everything else by its DISTANCE from that target — not by how much space the text",
  "gives it, and not by how technical or specific it sounds:",
  "  0.6-0.8  a co-equal or closely adjacent target named in its own right",
  ...
  "",
  "A mechanism OF the primary target never outscores the primary target. If a description is",
  "about multiple sclerosis and dwells at length on remyelination, microglia and the blood-brain",
  "barrier, then multiple sclerosis is 1.0 and those three are means to it (~0.4) — even",
  "though the text spends more words on them than on the disease itself.",
  "",
  "Deduplicate near-identical concepts (keep the most canonical phrasing).",
  "",
  "For each concept, also return a GLOSS: what the description specifically means by THIS concept",
  "— its own sense or scope, in its words, quoting or closely paraphrasing the text, in AT MOST",
  "15 words. The canonical `term` is for a taxonomy lookup and deliberately strips this nuance;",
  "the gloss keeps it. For a description about antibody-drug conjugates that mentions lysosomes,",
  'the concept term is "lysosomes" and the gloss is "lysosomal processing of ADC linkers", NOT the',
  "bare organelle.",
  "",
  "A gloss must characterize THE CONCEPT ITSELF, in the description's own language. If the text",
  "only NAMES a concept, or uses it as the SETTING or TARGET for something else, and says nothing",
  "about the concept as such, LEAVE ITS GLOSS EMPTY — do not describe it through a neighbour.",
  'Example: for a description about "immune checkpoint blockade resistance in solid tumors", the',
  'concept "solid tumors" gets NO gloss (the text says nothing about solid tumors themselves —',
  'they are only the setting). Do NOT write "context for immune checkpoint blockade resistance":',
  "that describes checkpoint blockade, not solid tumors. A gloss may state the role or",
  "significance of a concept, but must NOT name another concept from your list inside it. If it",
  'would, drop that trailing reference: for "interstitial lung disease" write "leading cause of',
  'mortality", NOT "leading cause of mortality in systemic sclerosis"; if nothing concept-specific',
  "remains, omit.",
  "",
  "Prefer NO gloss over a borrowed one. An absent gloss is not a failure — it is correct whenever",
  "the text does not characterize the concept in its own words. NEVER invent context.",
  "",
  ...
  "description is after. Follow these rules exactly, so titles read consistently across searches:",
  ...
  "  - If the description names the ORGANIZATION behind it (a company, foundation, institute,",
  '    agency, lab, or center), prefix it VERBATIM and join with " — " (space, em dash, space):',
